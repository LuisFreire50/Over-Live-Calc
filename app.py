...
with st.expander("🔬 Diagnóstico técnico (matriz de placares restantes e detalhes)", expanded=False):
    ...
    # Gradient manual (sem matplotlib) — combina com o tema dark
    def _heatmap_css(values: pd.DataFrame) -> pd.DataFrame:
        vmin = float(values.values.min())
        vmax = float(values.values.max())
        rng = vmax - vmin if vmax > vmin else 1.0

        def style_cell(v: float) -> str:
            t = (v - vmin) / rng  # 0 → 1
            # Interpola do fundo escuro (#1A1F2C) até verde-trader (#00C49A)
            r = int(26 + t * (0 - 26))
            g = int(31 + t * (196 - 31))
            b = int(44 + t * (154 - 44))
            text_color = "#FAFAFA" if t < 0.55 else "#0E1117"
            return f"background-color: rgb({r},{g},{b}); color: {text_color};"

        return values.map(style_cell)

    styled = df.style.apply(_heatmap_css, axis=None).format("{:.2f}")
    st.dataframe(styled)
